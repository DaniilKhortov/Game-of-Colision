...
while True:
    ...
    for event in events:
        if button.state != "pressed" and button.state != "hover":
                mm.add(button, 0, vmax=1) #vmax is 1 by default
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:  # Ліва кнопка миші
                # Створення нового автомобіля на місці натискання
                new_box = Box(event.pos[0], event.pos[1], 5, BOX_MASS1, 0.99, -0.01)
                boxes.append(new_box)
                selectedBox = new_box  # Новостворений об'єкт автоматично стає вибраним
            
            elif event.button == 3 and selectedBox:  # Права кнопка миші
                # Встановлення вектора для вибраного об'єкта
                selectedBox.set_vector(*event.pos)
        
        elif event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RETURN and not inputLocked:  # Запуск руху
                inputLocked = True
                readyToMove = True
                for box in boxes:
                    box.readyToMove = True
            
            elif event.key == pygame.K_LSHIFT:  # Скидання до початкового стану
                inputLocked = False
                readyToMove = False
                for box in boxes:
                    box.reset()
    ...
    # Оновлення стану об'єктів
    if readyToMove:
        for box in boxes:
            box.update()
        
        # Обробка зіткнень між усіма парами об'єктів
