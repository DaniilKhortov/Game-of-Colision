...
class Box:
    ...
    def update(self):
        if self.readyToMove:
            ...
            if self.vx != 0 or self.vy != 0 and self.speed>0:
                target_angle = math.degrees(math.atan2(-self.vy, self.vx))

                # Обчислення різниці між поточним і цільовим кутом
                angle_diff = (target_angle - self.angle + 180) % 360 - 180

                # Якщо різниця більше порога, повертаємо
                rotation_threshold = 5  # Поріг у градусах
                if abs(angle_diff) > rotation_threshold:
                    self.angle += 1 * math.copysign(1, angle_diff)
                else:
                    self.angle = target_angle  # Фіксуємо кут
    ...
    def draw(self, screen):
        ...
        if self.selected:
            pygame.draw.rect(screen, (255, 0, 0), 
                            pygame.Rect(int(self.x - SIZEX / 2), int(self.y - SIZEY / 2), SIZEX, SIZEY), 2)
    ...
    def is_clicked(self, pos):
        return (self.x - SIZEX / 2 <= pos[0] <= self.x + SIZEX / 2 and
                self.y - SIZEY / 2 <= pos[1] <= self.y + SIZEY / 2)   
    def reset(self):
        self.x = self.start_x
        self.y = self.start_y
        self.vx = 0
...
box3 = Box(3 * WIDTH // 5, HEIGHT // 3, 7, BOX_MASS2, 0.99, 0)
