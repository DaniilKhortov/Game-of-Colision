...
class Box:
    def __init__(self, x, y, speed, mass, friction, acceleration, sprite, sizeX, sizeY):
        self.start_x = x
        self.start_y = y
        self.x = x
        self.y = y
...
def createWidget():
    ...
    def update_sprite(event):
        index = widgets.index([labelHeader, labelSpeed, labelAcceleration, labelType, labelSize, labelMass, inputAcceleration, inputSpeed, inputMass, inputSize, inputType])
        if typeVar.get() == 'Sedan':
            boxes[index].sprite = car_image
            inputSize.delete(0, tk.END)
            inputSize.insert(0, "4x2")
        elif typeVar.get() == 'Van':
            boxes[index].sprite = van_image
            inputSize.delete(0, tk.END)
            inputSize.insert(0, "4x2.1")
        elif typeVar.get() == 'Mouse':
            boxes[index].sprite = mouse_image
            inputSize.delete(0, tk.END)
...
def trigger_event(event_type):
    global inputLocked, readyToMove
    if event_type == "ENTER" and not inputLocked:
        ...
        for box in boxes:
            mass = parameters[boxes.index(box)][3].get() 
            speed = parameters[boxes.index(box)][0].get() 
            acceleration = parameters[boxes.index(box)][1].get()
            size = parameters[boxes.index(box)][2].get()
            x, y = size.split("x")
            x = float(x)
            y = float(y)
            box.sizeX = x*28
...
def pygame_event_handler():  
    global selectedBox, readyToMove, inputLocked, currentBG, mode
    ...
    for event in events:
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1 and len(boxes)<5:  
                new_box = Box(event.pos[0], event.pos[1], 8, BOX_MASS1, 0.99, -1/600, car_image, SIZEX, SIZEY)
        ...
        elif event.type == pygame.MOUSEBUTTONDOWN and not inputLocked and mode == "structure":
            if event.button == 1:  
                new_box = Box(event.pos[0], event.pos[1], 0, 999999999999, 0.99, -1/600, structureImage, 40, 40)
